#' Conflicts between the scalade and other packages
#'
...
scalade_conflicts <- function(core, include_internal_conflicts = FALSE) {
    ...
    tidy_names <- paste0("package:", core) # scalade_packages())
    conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

    conflict_funs <- purrr::imap(conflicts, confirm_conflict)
    conflict_funs <- purrr::compact(conflict_funs)

    structure(conflict_funs, class = "scalade_conflicts")
}

scalade_conflict_message <- function(x) {
    if (length(x) == 0) {
        return("")
    }
...
#' @export
print.scalade_conflicts <- function(x, ..., startup = FALSE) {
    cli::cat_line(scalade_conflict_message(x))
}
